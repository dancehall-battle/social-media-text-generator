...
async function generateCaption(battle) {
  ...
  if (battle.participants === '3') {
    const textsSecondWinner = generateTextsForWinner(battle.hasWinner[1]);
    const textsThirdWinner = generateTextsForWinner(battle.hasWinner[2]);
    instagramCaption += `, ${textsSecondWinner.instagram} and ${textsThirdWinner.instagram}`;
    twitterDescription += `, ${textsSecondWinner.twitter} and ${textsThirdWinner.twitter}`;
  }
